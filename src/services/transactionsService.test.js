...
const { debit, credit } = require('../utils/transactionUtils.js');
...
describe('transactionsService', () => {
  ...
  test('processes transactions and updates balance sheet', async () => {
    ...
    debit.mockImplementation((payer, amount, map) => {
      map.set(payer, map.get(payer) - amount);
    });

    credit.mockImplementation((payee, amount, map) => {
      map.set(payee, map.get(payee) + amount);
    });

    const result = await service.processPaymentAdjustments('dummy.csv');

    expect(mockDAL.getBalanceSheet).toHaveBeenCalled();
